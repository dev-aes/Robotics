                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <table id="example1" class="table table-striped table-bordered" style="width:100%">
                                    ...
                                    $q = $conn->query("SELECT * FROM `section_adviser` WHERE `section_adviser_s_id` = '$u_id' AND `section_adviser_u_id`='$section_id'") or die(msqli_error());
                                    $f = $q->fetch_array();
                                    $section_adviser_u_id = $f['section_adviser_u_id'];
                                    $q_e = $conn->query("SELECT * FROM `user_account` WHERE `position`='Student' AND `student_section`='$section_adviser_u_id'") or die(mysqli_error());
                                    while($f_e=$q_e->fetch_array()){
                                        $u_id_stu = $f_e['u_id'];
                                        $sqlc="SELECT count(*) AS totalc FROM `chat` WHERE `chat_user_id` = '$u_id_stu' AND `chat_user` = '$u_id_stu' AND `chat_to`='$u_id' AND `view`=''";
                                        $resultc=mysqli_query($conn,$sqlc);
                                        $datac=mysqli_fetch_assoc($resultc);
                                        ?>
                                        <tr>
