      ...
      <div class="main-panel">
        <div class="content-wrapper">
		  ...
		  <!-- Content Left -->
            ...
            <div class="col-md-12 grid-margin transparent">
                ...
                $sql1="SELECT count(*) AS total1 FROM `enable_history_lab` WHERE `enable_history_id` = '7' AND `enable_history_fac`='$u_id' AND `enable_history_sec`='$section_name'";
                ...
                if($count1>='1'){
                    ?>
                    <a href="lab-6-list.php?disabled=1&week=7&section_name=<?php echo $_GET['section_name']?>&disable" class="btn btn-primary btn-sm" >Disable this Week</a>
                <?php }else{
                    ?>
                    <a href="lab-6-list.php?disabled=0&week=7&section_name=<?php echo $_GET['section_name']?>&disable" class="btn btn-primary btn-sm" >Enable this Week</a>
                    <?php
                }
                if(isset($_GET['disable'])){
                    ...
                    $section_name	 =$_GET['section_name'];
                    if($disabled==1){
                        $result=$conn->query("DELETE FROM `enable_history_lab` WHERE `enable_history_id`='7' AND `enable_history_fac`='$u_id' AND `enable_history_sec`='$section_name'");
                        if($result){
                            echo
                                '<script>
					                function myFunction(){
						            ...
						            window.location = "lab-6-list.php?section_name='.$section_name.'";
					                });}
				                </script>';
                        }else{
                            echo'
                        ...
                        $sql ="INSERT INTO enable_history_lab VALUES(null,'7','$u_id','$section_name')";
		...
		<a href="excel.php" class="btn-success btn-sm btn" style="text-transform:capitalize;color:white;text-stroke:2px solid black;">
			<i class="nav-icon fas fa-file" ></i> Export Excel
                ...
                <a href="word/lab-6.docx" class="btn-dark btn-sm btn" download="Laboratory-Work-No.-6-Keypad.docx" style="text-transform:capitalize;color:white;text-stroke:2px solid black;">
        ...
        <tbody><?php 
					...
					while($f_e=$q_e->fetch_array()){ 
						$u_id1 = $f_e['u_id'];
						$q = $conn->query("SELECT * FROM `student_file` WHERE `student_lab_num`='6' AND `student_file_user`='$u_id1'") or die(msqli_error());
						$f = $q->fetch_array();
				?>
            <tr>
                ...
                <td>
                    <?php if(!empty($f['student_lab_date'])){ ?>
                        <a href="view-update.php?user_id=<?php echo $u_id1?>&student_file_id=<?php echo $f['student_file_id']?>&section_name=<?php echo $_GET['section_name']?>&student_lab_num=6" class="btn bg-danger btn-sm">Update</a>
                    <?php }else{
                        ?>
