...
if(isset($_POST['signin'])){
    ...
    if(empty($email_err) && empty($password_err)){
        ...
        $sql = "SELECT u_id, email, password FROM user_account WHERE email =?";
        ...
        if($stmt = mysqli_prepare($conn, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    // Bind result variables 123123_Az
        ...
        <div class="row w-100 mx-0">
          <div class="col-lg-4 mx-auto">
            <div class="auth-form-light text-left py-5 px-4 px-sm-5">
              ...
              <h4>Hello Teachers! let's get started</h4>
              <h6 class="font-weight-light">Sign in to continue.</h6>
              <form class="pt-3"method = "POST" action = "<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype = "multipart/form-data" autocomplete="off" value="<?php echo $_POST["email"]?>">
                <div class="form-group"  <?php echo (!empty($email_err)) ? 'has-error' : ''; ?> >
                  <input type="email" class="form-control form-control-lg" name="email"id="exampleInputEmail1" placeholder="Email">
				  <span class="help-block"  style="color:#DC143C;"><?php echo $email_err; ?></span>
                </div>
