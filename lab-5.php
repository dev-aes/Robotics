            ...
            <div class="col-md-12 grid-margin">
              <div class="row">
                <div class="col-12 col-xl-8 mb-4 mb-xl-0"> 
                  ...
                  <h6 class="font-weight-normal mb-0">This is the Laboratory for <span class="text-primary"><b>Week 5</b></span></h6>
                </div>
              </div>
            </div>
          </div>	
		  ...
		  <!-- Content Left -->
            ...
            <div class="col-md-12 grid-margin transparent">
                  <div class="card">
                    <div class="card-body">
					<?php
						$q = $conn->query("SELECT * FROM `student_file` WHERE `student_file_user` = '$u_id'AND `student_lab_num`='4'") or die(msqli_error());
						$f = $q->fetch_array();
					?>
                      <h4 class="card-title">Week 5 - Laboratory Work 4 - Control Components <text style="float:right;"> <b style="color:red;">Score:</b>
					  <?php 
					  ...
					  ?></text></h4>
					  <a href="word/lab-4.docx" download="Laboratory-Work-No.-4-Control-Components.docx">Download this activity for Laboratory 4 for Week 5</a>
					  
                    </div>
					<div class="card-body">
					<br>
                        <?php
                        $qw = $conn->query("SELECT * FROM `multiple_file` WHERE `multiple_file_u_id` = '$u_id' AND `multiple_file_week`='4'") or die(mysqli_error());
                        while($fw=$qw->fetch_array()){
                            ...
                            if(empty($f['student_lab_status'])){ ?>
                                ...
                                <?php }else{
                                    $sql1="SELECT count(*) AS total1 FROM `multiple_file` WHERE `multiple_file_week` = '4' AND `multiple_file_u_id`='$u_id'";
                                    ...
                                    if($data1['total1']<=1){
                                        ?>
                                        <a href="lab-5.php?action=delete&student_file_id=<?php echo $fw['multiple_file_id'] ?>&student_file_user=<?php echo $f['student_file_user'] ?>" class="btn btn-primary btn-sm font-weight-medium auth-form-btn" >Delete</a><br><br>
                                    <?php }else{?>
                                        <a href="lab-5.php?action=delete&student_file_id=<?php echo $fw['multiple_file_id'] ?>" class="btn btn-primary btn-sm font-weight-medium auth-form-btn" >Delete</a><br><br>
                                        <?php
                                    }
                                }?>
                        ...
                        if(isset($_GET['action'])){
                            ...
                            $result1=$conn->query("DELETE FROM student_file WHERE student_file_user='$student_file_user' AND `student_lab_num`='4'");
                            if($result1){}
                            if($result){
                        ...
                        if(isset($_POST['upload_file'])){
                            ...
                            $sql="SELECT count(*) AS total FROM `multiple_file` WHERE `multiple_file_week` = '4' AND `multiple_file_u_id`='$u_id'";
                            $result=mysqli_query($conn,$sql);
                            $data=mysqli_fetch_assoc($result);
                            if($data['total']>='1'){
                                $sql ="INSERT INTO multiple_file VALUES(null,'4','$u_id','$file_name')";
                                if (mysqli_query($conn,$sql)) {
                                    echo '<script>
									function myFunction() {
										swal({
										title: "Success!",
										text: "Successfully Submitted Laboratory 4",
									    icon: "success",
										type: "successs"
										}).then(function() {
										window.location = "lab-5.php";
                                ...
                                $sql ="INSERT INTO multiple_file VALUES(null,'4','$u_id','$file_name')";
                                if (mysqli_query($conn,$sql)) {}
                                $sql1 ="INSERT INTO student_file VALUES(null,'$u_id','','','4','$date','0','Pending')";
