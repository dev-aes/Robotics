		  ...
		  <!-- Content Left -->
            ...
            <div class="col-md-12 grid-margin transparent">
                  <div class="card">
                    <div class="card-body">
					<?php
						$q = $conn->query("SELECT * FROM `student_file` WHERE `student_file_user` = '$u_id' AND `student_lab_num`='9'") or die(msqli_error());
						$f = $q->fetch_array();
					?>
					  ...
					  ?></text></h4>
					  <a href="word/weekly.docx" download="Weekly-Accomplishment-Report-1.docx">Download this activity for  Week 10</a>
					  
                    </div>
					<div class="card-body">
					<br>
                        <?php
                        $qw = $conn->query("SELECT * FROM `multiple_file` WHERE `multiple_file_u_id` = '$u_id' AND `multiple_file_week`='9'") or die(mysqli_error());
                        while($fw=$qw->fetch_array()){
                            ...
                            if(empty($f['student_lab_status'])){ ?>
                                ...
                                <?php }else{
                                    $sql1="SELECT count(*) AS total1 FROM `multiple_file` WHERE `multiple_file_week` = '9' AND `multiple_file_u_id`='$u_id'";
                                    ...
                                    if($data1['total1']<=1){
                                        ?>
                                        <a href="lab-10.php?action=delete&student_file_id=<?php echo $fw['multiple_file_id'] ?>&student_file_user=<?php echo $f['student_file_user'] ?>" class="btn btn-primary btn-sm font-weight-medium auth-form-btn" >Delete</a><br><br>
                                    <?php }else{?>
                                        <a href="lab-10.php?action=delete&student_file_id=<?php echo $fw['multiple_file_id'] ?>" class="btn btn-primary btn-sm font-weight-medium auth-form-btn" >Delete</a><br><br>
                                        <?php
                                    }
                                }?>
                        ...
                        if(isset($_GET['action'])){
                            ...
                            $result1=$conn->query("DELETE FROM student_file WHERE student_file_user='$student_file_user' AND `student_lab_num`='9'");
                            if($result1){}
                            if($result){
                        ...
                        if(isset($_POST['upload_file'])){
                            ...
                            $sql="SELECT count(*) AS total FROM `multiple_file` WHERE `multiple_file_week` = '9' AND `multiple_file_u_id`='$u_id'";
                            $result=mysqli_query($conn,$sql);
                            $data=mysqli_fetch_assoc($result);
                            if($data['total']>='1'){
                                $sql ="INSERT INTO multiple_file VALUES(null,'9','$u_id','$file_name')";
                                if (mysqli_query($conn,$sql)) {
                                    echo '<script>
									function myFunction() {
										swal({
										title: "Success!",
										text: "Successfully Submitted Laboratory 9",
									    icon: "success",
										type: "successs"
										}).then(function() {
										window.location = "lab-10.php";
                                ...
                                $sql ="INSERT INTO multiple_file VALUES(null,'9','$u_id','$file_name')";
                                if (mysqli_query($conn,$sql)) {}
                                $sql1 ="INSERT INTO student_file VALUES(null,'$u_id','','','9','$date','0','Pending')";
